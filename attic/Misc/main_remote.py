    ...
    while time.time() - start < timeout:
        try:
            ...
            if name in status.deployments:
                ...
                logger.error(f"Deployment status: {deployment.status}")
                logger.error(f"Deployment message: {deployment.message}")
                logger.error(f"Deployment replica status: {deployment.replica_states}")
...
def check_deployment_health(name, timeout=30):
    ...
    while time.time() - start < timeout:
        try:
            ...
            if name in status.deployments:
                deployment = status.deployments[name]
                logger.info(f"Full deployment status: {deployment}")
                logger.info(f"Replica states: {deployment.replica_states}")
                
                if deployment.status == "UNHEALTHY":
                    logger.error(f"Error message: {deployment.message}")
                    logger.error(f"Last update: {deployment.last_update_time}")
        ...
        deployment_options = {
            ...
            "ray_actor_options": {
                ...
                "runtime_env": {
                    "env_vars": {
                        "RAY_CLIENT_MODE": "0",
                        #"working_dir": "s3://ntoplabs-0001/main.zip"
                    },
                }
            }
        }
