...
def _configure_key_pair(config):
    #cli_logger._set_verbosity(2)  # Force debug/verbose logs
    ...
    if "ssh_private_key" in config["auth"]:
        ...
        # Ensure `KeyName` is provided in `node_config` if `UserData` is not present
        for node_type_key, node_type in node_types.items():
            node_config = node_type["node_config"]
            if "UserData" not in node_config:
                if "KeyName" not in node_config:
                    cli_logger.abort(
                        f"KeyName is missing for node type '{node_type_key}' "
                        f"and no UserData is provided for SSH setup."
                    )
                cli_logger.verbose(
                    f"Using existing KeyName: {node_config['KeyName']} for node type '{node_type_key}'."
                )
        return config

    # Default to creating or using an existing key pair
    for node_type_key in node_types:
    ...
    # Attempt to get or create a key pair
    MAX_NUM_KEYS = 600
    ...
    for i in range(MAX_NUM_KEYS):
        ...
        if key and os.path.exists(key_path):
            cli_logger.verbose(f"Using existing key pair: {key_name}")
            break
        ...
        if not key and not os.path.exists(key_path):
            ...
            # Save the private key with proper permissions
            with open(key_path, "w", opener=partial(os.open, mode=0o600)) as f:
                f.write(key.key_material)
            break

    # Abort if no key was found or created
    if not key:
        cli_logger.abort(
            "No available key pairs found or created. Ensure your account does not "
            "exceed the AWS key pair limit ({} keys).", MAX_NUM_KEYS
        )

    # Ensure the private key file exists
    cli_logger.doassert(
        os.path.exists(key_path),
        "Private key file {} not found for key pair {}.",
        key_path, key_name,
    )
    assert os.path.exists(key_path), f"Private key file {key_path} not found for key pair {key_name}."

    # Update the config with the private key path and key name
    config["auth"]["ssh_private_key"] = key_path
    for node_type_key, node_type in node_types.items():
        node_config = node_type["node_config"]
        cli_logger.verbose(f"Final KeyName for node type '{node_type_key}': {node_config.get('KeyName')}")
        if "KeyName" not in node_config:
            cli_logger.verbose(f"Setting KeyName for node type '{node_type_key}' to: {key_name}")
